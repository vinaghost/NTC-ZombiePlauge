...
#define DAMAGE_A 2.0 // 45 for zombie
#define DAMAGE_B 65 // 60 for zombie
...
new g_Event_Aeolis, g_ShellId, g_SmokePuff_SprId, g_SmokePuff_SprId2
new g_item;
public plugin_init()
{
	...
	g_item = zp_weapons_register("Aeolis", 25, ZP_PRIMARY, ZP_WEAPON_AP);
...
{
    if(ItemID == g_item) Get_Aeolis(id)
}
public zp_fw_wpn_remove(id, ItemID )
{
    if(ItemID == g_item) Remove_Aeolis(id)
}
...
public fw_TraceAttack_World(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	...
	SetHamParamFloat(3, Damage * DAMAGE_A)
...
public fw_TraceAttack_Player(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	...
	if(g_FireAmmo[Attacker] < FIRE_AMMO)
	{
		g_FireAmmo[Attacker] = min(g_FireAmmo[Attacker] + FIRE_INCBY, FIRE_AMMO)
		Check_FireAmmoHud(Attacker, 1, 0)
	}
