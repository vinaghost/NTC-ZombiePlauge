...
#define CSW_JANUS3 CSW_UMP45
#define weapon_janus5 "weapon_ump45"
#define PLAYER_ANIMEXT "carbine"
#define JANUS3_OLDMODEL "models/w_ump45.mdl"
...
new g_Had_Janus3, g_Janus3_Mode[33], g_Janus3_Clip[33], g_BulletCount[33], g_ChangingMode, Float:g_Recoil[33][3]
new g_Event_Janus3, g_Msg_WeaponList, g_SmokePuff_SprId, g_RifleShell_Id, g_ham_bot
new g_janus3
public plugin_init()
{
	...
	g_janus3 = zp_weapons_register("Janus 3", 40, ZP_PRIMARY, ZP_WEAPON_AP)
...
{
    if(ItemID == g_janus3) Get_Janus3(id)
}
...
public zp_user_infected_post(id) Remove_Janus3(id)
public zp_user_humanized_post(id) Remove_Janus3(id)
...
public Get_Janus3(id)
{
	...
	Set_BitVar(g_Had_Janus3, id)
	UnSet_BitVar(g_ChangingMode, id)
	fm_give_item(id, weapon_janus5)

	Give_RealAmmo(id, CSW_JANUS3)

	static Ent; Ent = fm_get_user_weapon_entity(id, CSW_JANUS3)
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	...
	if(get_user_weapon(id) == CSW_JANUS3 && Get_BitVar(g_Had_Janus3, id))
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (!is_user_connected(invoker))
		return FMRES_IGNORED
	if(get_user_weapon(invoker) != CSW_JANUS3 || !Get_BitVar(g_Had_Janus3, invoker))
		return FMRES_IGNORED
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, JANUS3_OLDMODEL))
	{
		...
		if(Get_BitVar(g_Had_Janus3, iOwner))
		{
...
public fw_CmdStart(id, uc_handle, seed)
{
	if(!is_user_alive(id))
		return FMRES_IGNORED
	if(!Get_BitVar(g_Had_Janus3, id) || get_user_weapon(id) != CSW_JANUS3)
		return FMRES_IGNORED
	...
	if((PressButton & IN_ATTACK2))
	{
		...
		Set_Player_NextAttack(id, CSW_JANUS3, CHANGE_TIME)
...
public TurnOff_JTS2(id)
{
	...
	Set_BitVar(g_ChangingMode, id)
	Set_Player_NextAttack(id, CSW_JANUS3, CHANGE_TIME - 1.0)
...
public fw_Weapon_PrimaryAttack_Post(Ent)
{
	...
	if(Get_BitVar(g_Had_Janus3, id) && g_Janus3_Mode[id] == JANUS3_JANUS)
	{
		static Float:Push[3]
...
public fw_Item_AddToPlayer_Post(Ent, id)
{
	...
	write_string(Get_BitVar(g_Had_Janus3, id) ? "weapon_janus3" : "weapon_ump45")
	...
	write_byte(Get_BitVar(g_Had_Janus3, id) ? CSW_JANUS3 : CSW_UMP45) // WeaponID
	write_byte(0) // Flags
	message_end()
...
public fw_Item_PostFrame(ent)
{
	...
	static bpammo; bpammo = cs_get_user_bpammo(id, CSW_JANUS3)
	...
	if(fInReload && flNextAttack <= 0.0)
	{
		...
		cs_set_user_bpammo(id, CSW_JANUS3, bpammo - temp1)
...
public fw_Weapon_Reload_Post(ent)
{
	...
	{ // Reload
		...
		//Set_Player_NextAttack(id, CSW_JANUS3,
	}
...
public fw_TraceAttack_World(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	if(!is_user_connected(Attacker))
		return HAM_IGNORED
	if(get_user_weapon(Attacker) != CSW_JANUS3 || !Get_BitVar(g_Had_Janus3, Attacker))
		return HAM_IGNORED
	...
	SetHamParamFloat(3, g_Janus3_Mode[Attacker] == JANUS3_JANUS ? float(DAMAGE_B) : float(DAMAGE_A))
