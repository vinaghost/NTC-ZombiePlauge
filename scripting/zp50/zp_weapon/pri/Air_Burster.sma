...
public plugin_init()
{
	...
	g_item = zp_weapons_register("Air Burster", 25, ZP_PRIMARY, ZP_WEAPON_AP);

}
public zp_fw_wpn_select_post(id, ItemID)
{
    if(ItemID == g_item) Get_Cannon(id)
}
public zp_fw_wpn_remove(id, ItemID )
{
    if(ItemID == g_item) Remove_Cannon(id)
}
