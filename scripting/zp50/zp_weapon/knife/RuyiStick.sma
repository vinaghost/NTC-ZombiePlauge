...
public plugin_init()
{
	...
	g_item = zp_weapons_register("Ruyi Stick", 20, ZP_KNIFE, ZP_WEAPON_AP);
}
...
{
    if(ItemID == g_item) Get_RuyiStick(id)
}
public zp_fw_wpn_remove(id, ItemID )
{
    if(ItemID == g_item) {
    	UnSet_BitVar(g_Had_RuyiStick, id)
...
public Get_RuyiStick(id)
{
	...
	{
		cs_set_player_view_model(id, CSW_KNIFE, MODEL_V)
		cs_set_player_weap_model(id, CSW_KNIFE, MODEL_P)
