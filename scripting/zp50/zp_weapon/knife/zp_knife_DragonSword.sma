...
public plugin_precache()
{
	...
	g_DragonSword = zp_weapons_register("Dragon Sword", 9000, ZP_KNIFE, ZP_WEAPON_MONEY)
}

public zp_fw_wpn_select_post(id, ItemID)
{
    if(ItemID == g_DragonSword) get_dragonsword(id)
}
public zp_fw_wpn_remove(id, ItemID )
{
    if(ItemID == g_DragonSword) remove_dragonsword(id)
}

public zp_user_infected_post(id) remove_dragonsword(id)
public zp_user_humanized_post(id) remove_dragonsword(id)
