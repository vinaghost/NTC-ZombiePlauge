...
new cvar_money_assassin_ignore, cvar_money_sniper_ignore
...
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_ASSASSIN, LibType_Library))
		cvar_money_assassin_ignore = register_cvar("zp_money_assassin_ignore", "0")
	...
	if (LibraryExists(LIBRARY_SNIPER, LibType_Library))
		cvar_money_sniper_ignore = register_cvar("zp_money_sniper_ignore", "0")
...
public zp_fw_core_infect_post(id, attacker)
{
	...
	if (is_user_connected(attacker) && attacker != id && get_pcvar_num(cvar_money_human_infected) > 0)
		cs_set_user_money(attacker, cs_get_user_money(attacker) + get_pcvar_num(cvar_money_human_infected))
}
...
public fw_TakeDamage_Post(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (zp_core_is_zombie(attacker) && !zp_core_is_zombie(victim))
	{
		...
		if (get_pcvar_num(cvar_money_damage) > 0)
		{
			...
			{
				cs_set_user_money(attacker, cs_get_user_money(attacker) + (get_pcvar_num(cvar_money_damage) * how_many_rewards));
