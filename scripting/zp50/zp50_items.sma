...
show_ap_items_menu(id)
{
	...
	// Title
	formatex(menu, charsmax(menu), "Nâng cấp vật phẩm^nBạn đang có %d AP", zp_ammopacks_get(id)
);
...
show_money_items_menu(id)
{
	...
	// Title
	formatex(menu, charsmax(menu), "Item Extra^nBạn đang có $ %d", cs_get_user_money(id));
