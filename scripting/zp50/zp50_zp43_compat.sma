...
enum _:TOTAL_FORWARDS
{
	...
	FW_EXTRA_AP_ITEM_SELECTED,
	FW_EXTRA_MONEY_ITEM_SELECTED,
	FW_USER_UNFROZEN,
	FW_USER_LAST_ZOMBIE,
	FW_USER_LAST_HUMAN
}
...
public plugin_init()
{
	...
	g_Forwards[FW_EXTRA_AP_ITEM_SELECTED] = CreateMultiForward("zp_extra_ap_item_selected", ET_CONTINUE, FP_CELL, FP_CELL)
	g_Forwards[FW_EXTRA_MONEY_ITEM_SELECTED] = CreateMultiForward("zp_extra_money_item_selected", ET_CONTINUE, FP_CELL, FP_CELL)
...
{
	ExecuteForward(g_Forwards[FW_EXTRA_AP_ITEM_SELECTED], g_ForwardResult, id, itemid)
	if (g_ForwardResult >= ZP_PLUGIN_HANDLED)
	{
		// Item purchase was blocked, restore player's money/ammo packs
		new item_cost = zp_ap_items_get_cost(itemid)
...
{
	ExecuteForward(g_Forwards[FW_EXTRA_MONEY_ITEM_SELECTED], g_ForwardResult, id, itemid)
	if (g_ForwardResult >= ZP_PLUGIN_HANDLED)
	{
		// Item purchase was blocked, restore player's money/ammo packs
		new item_cost = zp_money_items_get_cost(itemid)
...
public plugin_natives()
{
	...
	register_native("zp_force_buy_extra_ap_item", "force_buy_extra_ap_item")
	register_native("zp_force_buy_extra_money_item", "force_buy_extra_money_item")
	...
	register_native("zp_register_extra_ap_item", "register_extra_ap_item")
	register_native("zp_register_extra_money_item", "register_extra_money_item")
	
	
	register_native("zp_register_zombie_class", "native_register_zombie_class")
	
	
	register_native("zp_get_extra_ap_item_id", "native_get_extra_ap_item_id")
	register_native("zp_get_extra_money_item_id", "native_get_extra_money_item_id")
...
public force_buy_extra_ap_item(plugin_id, num_params)
{
	new id = get_param(1)
	...
	return zp_ap_items_force_buy(id, itemid, ignorecost);
}

public force_buy_extra_money_item(plugin_id, num_params)
{
	new id = get_param(1)
	...
	new itemid = get_param(2)
	new ignorecost = get_param(3)
	return zp_money_items_force_buy(id, itemid, ignorecost);
}
...
public register_extra_ap_item(plugin_id, num_params)
{
	...
	new itemid = zp_ap_items_register(name, cost)
...
public register_extra_money_item(plugin_id, num_params)
{
	if (!LibraryExists(LIBRARY_EXTRAITEMS, LibType_Library))
		return -1;
	...
	new itemid = zp_money_items_register(name, cost)
	if (itemid < 0) return itemid;
...
public native_get_extra_ap_item_id(plugin_id, num_params)
{
	if (!LibraryExists(LIBRARY_EXTRAITEMS, LibType_Library))
		return -1;
	...
	new name[32]
	get_string(1, name, charsmax(name))
	return zp_ap_items_get_id(name);
}

public native_get_extra_money_item_id(plugin_id, num_params)
{
	if (!LibraryExists(LIBRARY_EXTRAITEMS, LibType_Library))
		return -1;
	...
	new name[32]
	get_string(1, name, charsmax(name))
	return zp_money_items_get_id(name);
}
