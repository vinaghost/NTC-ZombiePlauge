...
public ShowHUD(taskid)
{
	...
	if (player != ID_SHOWHUD)
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync, "Level: %d - Cấp bậc: %s^nName: %s^nHP: %d - Class: %s^nAP: %d - Money: %d", level, RANKS[level], player_name, get_user_health(player), class_name, zp_ammopacks_get(player), cs_get_user_money(player) )
	...
	else
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync, "Level: %d - Cấp bậc: %s^nHP: %d - Class: %s^nAP: %d - Money: %d", level, RANKS[level],get_user_health(player), class_name, zp_ammopacks_get(player), cs_get_user_money(player))
	}
}
