...
#define PLUGIN "[ZP] Weapon: Secondary default"
#define VERSION "1.0"
...
public plugin_init() {
	...
	for(new i =0 ; i < 6; i++ )
	{
		...
		primary[i] = zp_weapons_register(WEAPONNAMES[weaponid], 0, ZP_PRIMARY)
	}
}
...
give_weapon(id, index) {
	new weaponid = get_weaponid(secondary_items[index])
	
	give_item(id, secondary_items[index])
