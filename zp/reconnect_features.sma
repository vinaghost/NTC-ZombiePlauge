...
public task_delay_kill(id)
{
	...
	client_print(id, print_chat, "[Thông báo] Không thể sống lại được")
}
...
public client_putinserver(id)
{
	...
	for(new i; i < MAX_STORED; i++)
	{
		...
		if( equal(g_StoredInfos[i][StoreSteamId], szSteamId, strlen(szSteamId)) )
		{
			...
			if(!(get_user_flags(id)&ADMIN_SILENT) )
			{
				static szName[32]
				get_user_name(id, szName, 31)
				
				set_hudmessage(RED, GREEN, BLUE, -1.0, 0.35, 2, 3.0, 10.0, 0.1, 0.2, -1)
				show_hudmessage(0, "Buồn ! %s retry !!!!!!!!!!! ", szName)
...
public task_print_player(id)
{
	...
	{
		client_print(id, print_chat, "[Thông báo] Giữ nguyên mọi thứ như trước retry")
	}
}
